...
outer.prototype.odd = function(terms, opt, clean, next) {
    ...
    if (u.isConsonant(opt.fin) && u.isVowExA(opt.beg)) {
        var terms_e = terms.map(function(term) { return {stem: [term, c.e].join(''), raw: term, var: 'e'} });
        var terms_H = terms.map(function(term) { return {stem: [term, c.visarga].join(''), raw: term, var: 'H'} });
        odds = odds.concat(terms_e, terms_H);
        // log('OOOO', terms_H)
    }
    // log('FIN', opt.fin, 'BEG', opt.beg);
    else if (opt.fin == c.o && inc(c.soft, opt.beg)) {
        var terms_o = terms.map(function(term) { return {stem: [u.wolast(term), c.visarga].join(''), raw: term, var: 'o'}  });
        // log('OOOO', terms_o)
        odds = odds.concat(terms_o);
    }
    else if (opt.fin == c.A && (inc(c.soft, opt.beg) || inc(c.allvowels, opt.beg))) {
        var terms_A = terms.map(function(term) { return  {stem: [term, c.visarga].join(''), raw: term, var: 'A'}   });
        // log('OOOO', terms_o)
        ...
        var terms_hard = terms.map(function(term) {
            ...
            return {stem: [hard, hard_fin, c.virama].join(''), raw: term, var: 'hard'} ;
        });
